...
function workflowByStateTotal()
{
	...
	#$query = "SELECT COUNT(*) AS count, GROUP_CONCAT(DISTINCT IF(key.value='in progress',NULLIF(teamparam.value,''),NULL) SEPARATOR ',') AS resources, SUM(key.value='completed') AS 'completed', SUM(key.value='in progress') AS inprogress, SUM(key.value='standby') AS 'standby', SUM(key.value='ready') AS ready, SUM(key.value='not started') AS notstarted, SUM(key.value='aborted') AS aborted FROM node INNER JOIN `key` ON key.node_id=node.id INNER JOIN `key` AS teamparam ON teamparam.node_id=node.id WHERE key.name='state' AND teamparam.name='team';";
	#$query = "SELECT COUNT(*) AS count, SUM(key.value='completed') AS 'completed', SUM(key.value='in progress') AS inprogress, SUM(key.value='standby') AS 'standby', SUM(key.value='ready') AS ready, SUM(key.value='not started') AS notstarted, SUM(key.value='aborted') AS aborted FROM node INNER JOIN `key` ON key.node_id=node.id WHERE key.name='state';";
...
{
	$txt = '';
	#$query = "SELECT node.name, count(*) AS 'count', SUM(key.value='aborted') AS aborted, SUM(key.value='completed') AS 'completed', SUM(key.value='standby') AS 'standby', SUM(key.value='in progress') AS inprogress, SUM(key.value='ready') AS ready, SUM(key.value='not started') AS notstarted FROM node INNER JOIN `key` ON key.node_id=node.id WHERE key.name='state' GROUP BY node.name ORDER BY node.name;";
