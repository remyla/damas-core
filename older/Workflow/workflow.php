...
function workflowByStateTotal()
{
	...
	#$query = "SELECT COUNT(*) AS count, GROUP_CONCAT(DISTINCT IF(key.value='running',NULLIF(teamparam.value,''),NULL) SEPARATOR ',') AS resources, SUM(key.value='done') AS 'done', SUM(key.value='running') AS running, SUM(key.value='standby') AS 'standby', SUM(key.value='ready') AS ready, SUM(key.value='not started') AS notstarted, SUM(key.value='stopped') AS stopped FROM node INNER JOIN `key` ON key.node_id=node.id INNER JOIN `key` AS teamparam ON teamparam.node_id=node.id WHERE key.name='state' AND teamparam.name='team';";
	#$query = "SELECT COUNT(*) AS count, SUM(key.value='done') AS 'done', SUM(key.value='running') AS running, SUM(key.value='standby') AS 'standby', SUM(key.value='ready') AS ready, SUM(key.value='not started') AS notstarted, SUM(key.value='stopped') AS stopped FROM node INNER JOIN `key` ON key.node_id=node.id WHERE key.name='state';";
...
{
	$txt = '';
	#$query = "SELECT node.name, count(*) AS 'count', SUM(key.value='stopped') AS stopped, SUM(key.value='done') AS 'done', SUM(key.value='standby') AS 'standby', SUM(key.value='running') AS running, SUM(key.value='ready') AS ready, SUM(key.value='not started') AS notstarted FROM node INNER JOIN `key` ON key.node_id=node.id WHERE key.name='state' GROUP BY node.name ORDER BY node.name;";
	$query = "SELECT name.value AS name, count(*) AS 'count', SUM(state.value='stopped') AS stopped, SUM(state.value='done') AS 'done', SUM(state.value='standby') AS 'standby', SUM(state.value='running') AS running, SUM(state.value='ready') AS ready FROM node INNER JOIN `key` AS state ON state.node_id=node.id INNER JOIN `key` AS name ON name.node_id=node.id AND name.name='label' WHERE state.name='state' GROUP BY name.value ORDER BY name.value;";
	$result = mysql_query($query);
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)){
		$txt .= '<task ';
		//array_shift($row);
...
{
	$txt = '';
	#$query = "SELECT teamparam.value AS name, count(*) AS 'count', SUM(stateparam.value='stopped') AS stopped, SUM(stateparam.value='done') AS 'done', SUM(stateparam.value='standby') AS 'standby', SUM(stateparam.value='running') AS running, SUM(stateparam.value='ready') AS ready, SUM(stateparam.value='not started') AS notstarted FROM node INNER JOIN `key` AS teamparam ON teamparam.node_id=node.id INNER JOIN `key` AS stateparam ON stateparam.node_id=node.id WHERE teamparam.name='team' AND node.name='".$name."' AND stateparam.name='state' GROUP BY teamparam.value ORDER BY teamparam.value;";
	# NEW
